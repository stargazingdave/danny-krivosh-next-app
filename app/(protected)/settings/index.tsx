...
import { SongDetails } from './components/SongDetails'; // optional component for details
...
export default function SongManager() {
    ...
    const handleDrop = (e: React.DragEvent, targetId: string) => {
        ...
        setSaving("saving");
        updateSongOrder(reordered.map((s) => s.id)).then(() => {
            setSaving("saved");
    ...
    return (
        <div className="h-full w-full flex flex-col overflow-auto bg-black text-white">
            ...
            <div className="flex w-full h-[calc(100vh-13rem)] overflow-hidden border-t border-white/10">
                {/* Sidebar */}
                <aside className="w-72 shrink-0 border-r border-white/10 bg-white/5 p-4 flex flex-col gap-4">
                    <button
                        onClick={() => {
                    ...
                    <div className="flex flex-col gap-2 overflow-y-auto">
                        {songs.map((song, index) => (
                            <div
                                ...
                                className={`flex items-center gap-3 p-2 rounded cursor-pointer hover:bg-white/10 transition ${selectedSongId === song.id ? 'bg-white/10' : ''
                            ...
                            >
                                ...
                                <div className="flex flex-col flex-1">
                                    ...
                                    <div className="flex gap-2 mt-1 text-xs text-gray-400">
                                        ...
                                        {song.definition && <span className="text-amber-500">{song.definition.toUpperCase()}</span>}
                                    </div>
                                </div>
                            </div>
                        ))}
